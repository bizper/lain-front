        ...
        <Dialog open={open} as="div" className="relative z-10 focus:outline-none" onClose={_ => setOpen(false)}>
            <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div className="flex min-h-full items-center justify-center p-4">
                    <DialogPanel
                        transition
                        className="w-full max-w-md overflow-auto max-h-[60vh] rounded-xl bg-white/5 p-6 backdrop-blur-2xl duration-300 ease-out data-[closed]:transform-[scale(95%)] data-[closed]:opacity-0"
                    >
                        <DialogTitle as="h3" className="text-base/7 font-medium text-white mb-2">
                            ...
                            <div className="flex items-center justify-between my-2 h-auto">
                                <Button onClick={playWholeAlbum} className='group'>
                                    <PlayIcon className="size-8 fill-white/60 group-hover:fill-white" />
                                </Button>
                                <Button onClick={_ => {
                                    setOpen(false)
                                    setAlbum(album)
                                    ...
                                    <InformationCircleIcon className="size-8 fill-white/60 group-hover:fill-white" />
                                </Button>
                            </div>
                            ...
                            album.songs.map((s, index) => (
                                <a key={s.id} className="block rounded-lg py-2 px-3 transition hover:bg-white/5" href="#" onClick={_ => {
                                    ...
                                    <div className="flex items-center justify-between">
                                        <p className={'font-semibold text-white mr-4'}>{`${index + 1}.${s.name}`}</p>
